...
export default async function StudentArchiveDetailPage({ params }: { params: Promise<{ schoolYearId: string }> }) {
  ...
  const { data: schoolYear } = await supabase
    ...
    .eq("id", schoolYearId)
    .eq("student_id", user.id) // students may only open their own archived years
    .single();
  ...
  return (
    <GradeArchiveView
      ...
      studentColor={profile.color ?? "#4A90D0"}
